...
Highways typically contribute a significant portion of the night lights observed from space (Oliver, n.d.). Therefore, it is important to avoid mistakenly identifying areas with reduced traffic as power outages. To streamline data acquisition, a shapefile of all Texas highways was downloaded from [Geofabrik](https://download.geofabrik.de/), rather than using the larger OpenStreetMap dataset. A similar approach was utilized to access subsetted housing data and stored in a GeoPackage containing only houses in the Houston metropolitan area.

Socioeconomic data is particularly helpful for understanding disparities among communities, households, and other population groups. Socioeconomic was obtained from [U.S. Census Bureau’s American Community Survey](https://www.census.gov/programs-surveys/acs) at the 2019 census tract level, which was more appropriate than collecting data for every household. The `ACS_2019_5YR_TRACT_48.gdb` stores many geospatial layers in a multi-file format that can be referenced with `st_layers()`. Each layer includes a subset of the fields described in the associated [metadata](https://www2.census.gov/geo/docs/maps-data/data/tiger/prejoined/ACSMetadata2011.txt). Geometric information for Texas was referenced specifically for the analysis.
