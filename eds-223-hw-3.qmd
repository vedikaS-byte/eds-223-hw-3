...
Winter Storm Uri was a powerful and catastrophic winter storm that devastated several communities in Texas during February 13-17 2021. The state was not prepared for a significant loss of power 
...
```{r}
## a set of maps comparing night light intensities before and after the first two storms
tm1 <- tm_shape(lights_day1) + tm_raster(title="Feb 7 (Pre-storm)", style = "quantile",  palette = c("darkblue", "blue","lightblue")) 
tm2 <- tm_shape(lights_day2) + tm_raster(title="Feb 16 (Post-storm)", style = "quantile",  palette = c("darkblue", "blue", "lightblue")) 

#tm3 <- tm_shape(diff_raster) + tm_raster(palette="-RdYlBu", title="Change")
tm4 <- tm_shape(houston_blackout) + tm_polygons() +
   tm_layout(main.title = "Blackout Map of Houston") + tm_basemap(server= "OpenStreetMap")

tmap_arrange(tm1,tm2, nrow = 1, ncol = 2)
tm4
...
tm_shape(buildings_affected_blackout_test) + tm_polygons(col = "blue")  + tm_layout(main.title = "Houses in Houston Affected By the Blackout") + tm_basemap(server= "OpenStreetMap") ## add just the buildings
...
acs_affected %>% filter(!(is.na(B19013e1))) %>%
tm_shape(acs_affected) + tm_polygons(col = "B19013e1", 
                                     palette = "inferno") + tm_layout(main.title = "Census Tracks in Houston Affected By the Blackout")  + tm_basemap(server= "OpenStreetMap") ## add just the buildings
...
limitation: 
- distance to hospital grid 
- subsystem bias in GIS 
-  
