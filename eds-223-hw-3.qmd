...
#houses <- houses %>% filter(type == "residential")
...
# re-project the cropped blackout dataset to EPSG:3083 (NAD83 / Texas Centric Albers Equal Area)
tm_shape(houston_blackout) + tm_polygons() + 
  #tm_title("Blackout Map of Houston", position = tm_pos_out("center")) 
...
tm_shape(acs_affected) + tm_polygons() + tm_layout(main.title = "Census Tracks in Houston Affected By the Blackout") + tm_basemap(server= "OpenStreetMap") ## add just the buildings
