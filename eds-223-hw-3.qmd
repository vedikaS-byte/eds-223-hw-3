...
14.  Use `st_filter()` to compare the blackout polygons
    ...
    highways, as well as removing any overlapping regions. Then, create
    a map of these areas outside the extent of highways.
...
tm_shape(blackout_not_highway) + tm_polygons() +  tm_layout(main.title = "Areas of Houston Blackout That Do NOT Overlap Higways") + tm_shape(highways_200) + tm_polygons() + tm_basemap(server= "OpenStreetMap") 
...
Identify homes that experienced blackouts by combining the locations of
homes and blackouts
