...
plot(st_geometry(blackout_not_highway), col = 'blue', add = TRUE)


# st_difference(highways_200, blackout_mask) # Order, which one goes first?
plot(blackout_not_highway)
tm_shape(blackout_not_highway) + tm_polygons() + tm_title("Areas of Houston Blackout That Do NOT Overlap Higways", position = tm_pos_out("center"))
...
buildings_affected_blackout <- st_filter(houses, blackout_not_highway, .predicate = st_intersects)

tm_shape(buildings_affected_blackout) + tm_polygons() + tm_title("Houses in Houston Affected By the Blackout", position = tm_pos_out("center"))
...
tm_shape(acs_affected) + tm_polygons() + tm_title("Census Tracks in Houston Affected By the Blackout", position = tm_pos_out("center"))
